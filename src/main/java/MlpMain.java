...
public class MlpMain {
    ...
    public static void main(String[] args) {
        ...
        Controlador de base
         */
        boolean isVertebralColumn = false;
        ...
        if (isVertebralColumn) {
            ...
            for (int epoch = 0; epoch < 10000; epoch++) {
                ...
                double taxaErroClassTest = (double) errClsTraining / total;
                System.out.printf("[TEST VERTEBRAL COLUMN] Epoch: %d - ApproxError: %.4f - ClassError: %d/%d (%.2f%%)%n",
                        epoch, approxErrTest, errClsTest, total, taxaErroClassTest * 100.0);
                System.out.println("\n");
            }
        } else {
            ...
            for (int epoch = 0; epoch < 10000; epoch++) {
                ...
                double taxaErroClssTest = (double) errClsTraining / total;
                System.out.printf("[TEST VERTEBRAL COLUMN] Epoch: %d - ApproxError: %.4f - ClassError: %d/%d (%.2f%%)%n",
                        epoch, approxErrTest, errClsTest, total, taxaErroClssTest * 100.0);
                System.out.println("\n");
            }
